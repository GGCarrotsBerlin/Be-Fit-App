...
import geojson from 'json!./new.geojson';
// import local components Filter and ForkMe
import Filter from './Filter';
//import ForkMe from './ForkMe';
function getDummyDataFromApiAsync() {
    ...
    /*return fetch('http://192.168.194.48:5050/1210/recommended_locations')
        .then(function(response) {
            return response.json()
          }).then(function(json) {
            console.log('parsed json', json)
        ...
        .then((responseJson) => {
            ...
            console.log(responseJson.movies[5]);
...
let config = {};
config.params = {
  center: [52.51068402,13.40589523],
  zoomControl: false,
  zoom: 13,
  maxZoom: 19,
  minZoom: 11,
...
config.tileLayer = {
  uri: 'https://api.mapbox.com/styles/v1/mevrgior/ciubbzfy2005a2iod1ptv4er8/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoibWV2cmdpb3IiLCJhIjoiUzBjTENPUSJ9.rl0EEFMb7iNt4MZqWsA90w',
  params: {
    minZoom: 11,
...
class Map extends Component {
  ...
  componentDidMount() {
      ...
      console.log(getDummyDataFromApiAsync());
      ...
      if (feature.properties && feature.properties.NAME && feature.properties.LINE) {
        ...
        if (subwayLineNames.length < 19) {
          ...
          if (this.state.geojson.features.indexOf(feature) === this.state.numEntrances - 1) {
            ...
            subwayLineNames.unshift('All places');
          }
        }

        // assemble the HTML for the markers' popups (Leaflet's bindPopup method doesn't accept React JSX)
        const popupContent = `<h3>${feature.properties.NAME}</h3>
          <strong>Keywords: </strong>${feature.properties.LINE}`;
